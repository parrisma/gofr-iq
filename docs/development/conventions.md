...
- **`docker/.env`**: Generated by `scripts/bootstrap.py`. Authoritative for `NEO4J_PASSWORD`, `VAULT_ROOT_TOKEN`, and derived vars like `GOFR_IQ_NEO4J_URI`.
- **`docker/.vault-init.env`**: Generated by `scripts/bootstrap.py`. Authoritative for `VAULT_TOKEN`.
...
| Path | Contents |
|------|----------|
| **`docker/.env`** | **SSOT: `GOFR_JWT_SECRET`, `VAULT_ROOT_TOKEN`, `NEO4J_PASSWORD`, `GOFR_IQ_OPENROUTER_API_KEY`** |
| `docker/.vault-init.env` | `VAULT_TOKEN`, `VAULT_UNSEAL_KEY`, bootstrap tokens |
