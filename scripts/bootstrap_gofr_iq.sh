...
usage() {
    echo "Usage: $0 [--tests]"
    echo ""
    echo "Idempotent bootstrap for gofr-iq so ./scripts/start-prod.sh can run cleanly." 
    echo ""
    echo "Options:"
...
if [ -z "${OPENROUTER_EXISTS}" ]; then
    ...
    echo "Remediation: ./scripts/start-prod.sh --fresh --openrouter-key <your-key>" >&2
    exit 1
fi
...
if [ -z "${NEO4J_PW_EXISTS}" ]; then
    ...
    echo "Remediation: ./scripts/start-prod.sh --fresh" >&2
    exit 1
fi

echo "Bootstrap complete. Next: ./scripts/start-prod.sh"
