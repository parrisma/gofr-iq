...
if [ "$SHOW_HELP" = true ]; then
    ...
    echo ""
    echo "REQUIREMENTS:"
    echo "  - Docker must be installed and running (for --prod or --infra-only)"
    echo "  - For --dev mode, you must be inside the devcontainer"
    echo "  - Vault infrastructure must be running and unsealed"
    echo "  - secrets/ directory must contain vault credentials"
    echo "  - OpenRouter API key must be in Vault (see docker/start-prod.sh)"
    echo ""
    echo "  If starting from scratch, use docker/start-prod.sh instead."
    echo "  See lib/gofr-common/scripts/readme.md for authentication setup."
    echo ""
    echo "EXAMPLES:"
