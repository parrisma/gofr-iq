...
#
# Usage:
#   ./docker/start-prod.sh              # Normal start (auto-starts Vault if needed)
#   ./docker/start-prod.sh --fresh      # Fresh install (stores secrets in Vault)
#   ./docker/start-prod.sh --reset      # Wipe app data and reinitialize
#   ./docker/start-prod.sh --nuke       # Full clean: remove images, volumes, then reset
#
# This script:
...
if [ "$FRESH_INSTALL" = true ]; then
    ...
    # Prefer: explicit flag -> existing Vault secret -> prompt.
